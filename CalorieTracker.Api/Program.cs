...
builder.Services.AddOpenApi("v1", options =>
{
    options.AddDocumentTransformer((document, context, cancellationToken) =>
    {
        document.Info = new OpenApiInfo
        {
            ...
            Version = "v1",
            Description = "API para seguimiento de calorías con análisis de nutrición mediante IA (Gemini).",
...
.WithDescription("Devuelve el estado actual y la versión de la API.")
...
.WithDescription("Analiza las calorías del alimento usando IA (Gemini) y registra el resultado.")
.Produces<object>(StatusCodes.Status200OK)
...
.WithDescription("Crea una cuenta de usuario con email y contraseña. Devuelve el ID del usuario creado.")
...
.WithDescription("Devuelve los datos de perfil del usuario autenticado (peso, altura, objetivo calórico, etc.).")
.Produces<object>(StatusCodes.Status200OK)
...
.WithDescription("Devuelve todos los alimentos registrados en el día indicado junto con el total de calorías. Formato de fecha: yyyy-MM-dd.")
.Produces<object>(StatusCodes.Status200OK)
.Produces(StatusCodes.Status400BadRequest)
.Produces(StatusCodes.Status401Unauthorized);
...
.WithSummary("Estadísticas nutricionales por rango de fechas")
.WithDescription("Devuelve estadísticas de calorías consumidas en el rango de fechas especificado. Útil para gráficos del dashboard.")
.Produces<object>(StatusCodes.Status200OK)
.Produces(StatusCodes.Status401Unauthorized);
...
// Transformer que añade el esquema de seguridad JWT Bearer al documento OpenAPI
