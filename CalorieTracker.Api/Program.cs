...
// Security validation: JWT Secret must be provided via environment variable or Key Vault — never in appsettings.json
var jwtSecret = builder.Configuration["JwtSettings:Secret"];
if (string.IsNullOrWhiteSpace(jwtSecret) || jwtSecret.Length < 32)
    throw new InvalidOperationException(
        "JwtSettings:Secret is missing or too short. " +
        "It must be at least 32 characters. Supply it via an environment variable, " +
        "Azure Key Vault, or dotnet user-secrets — never in appsettings.json.");
